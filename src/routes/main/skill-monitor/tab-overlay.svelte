...
<div class="rounded-lg border border-border/60 bg-card/40 p-4 space-y-4 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.02)]">
  <div>
    <h2 class="text-base font-semibold text-foreground">Overlay Area Visibility</h2>
    <p class="text-xs text-muted-foreground">
      Control whether each overlay section is shown or hidden (saved per profile)
    </p>
  </div>
  <div class="space-y-2">
    <div class="flex flex-wrap gap-2">
      ...
      >
        Skill CD Area: {showSkillCdGroup ? "Visible" : "Hidden"}
      </button>
      <button
        type="button"
      ...
      >
        Resource Monitor Area: {showResourceGroup ? "Visible" : "Hidden"}
      </button>
      <button
        type="button"
      ...
      >
        Character Attributes Area: {showPanelAttrGroup ? "Visible" : "Hidden"}
      </button>
      <button
        type="button"
      ...
      >
        Custom Panel Area: {showCustomPanelGroup ? "Visible" : "Hidden"}
      </button>
    </div>
    <p class="text-xs text-muted-foreground">
