...
<div class="space-y-6">
  <div class="rounded-lg border border-border/60 bg-card/40 p-4 space-y-4 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.02)]">
    <div class="flex items-center justify-between gap-3">
      <div>
        ...
        <p class="text-xs text-muted-foreground">Search by Buff name (supports icon and no-icon Buffs)</p>
      </div>
      <div class="flex items-center gap-3">
        <div class="text-xs text-muted-foreground">
          Selected: {monitoredBuffIds.length} Buffs / {monitoredBuffCategories.length} Categories
        </div>
        <button
          type="button"
  ...
  <div class="rounded-lg border border-border/60 bg-card/40 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.02)]">
    ...
    {#if buffAliasSectionExpanded}
      <div class="px-4 pb-4 space-y-4">
        <div class="space-y-2">
          <div class="text-xs text-muted-foreground">
            Configured aliases: {configuredBuffAliasIds.length}
          </div>
          <input
                ...
                <div class="flex items-center justify-between gap-3">
                  <div class="min-w-0">
                    ...
                    <div class="text-xs text-muted-foreground truncate">
                      Default: {getBuffDefaultName(buffAliasEditingBuffId)} | ID: {buffAliasEditingBuffId}
                    </div>
                  </div>
                  <button
                    type="button"
                ...
                <div class="rounded-md border border-border/60 bg-muted/20 p-3 space-y-2">
                  <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                      ...
                      <div class="text-xs text-muted-foreground truncate">
                        Default: {getBuffDefaultName(buffId)} | ID: {buffId}
                      </div>
                    </div>
                    <button
                      type="button"
        ...
        {:else}
          <div class="text-xs text-muted-foreground">
            No aliases configured. Search for a Buff above to set an alias.
          </div>
        {/if}
      </div>
    {/if}
  ...
  <div class="rounded-lg border border-border/60 bg-card/40 p-4 space-y-4 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.02)]">
    <div>
      ...
      <p class="text-xs text-muted-foreground">Switch between individual positioning and grouped layout — saved per profile</p>
    </div>
    <div class="flex flex-wrap gap-2">
      <button
        type="button"
    ...
    <label class="block text-xs text-muted-foreground max-w-md">
      Max no-icon Buffs visible: {textBuffMaxVisible}
      <input
        class="w-full mt-1"
        type="range"
        min="1"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-w-4xl">
      <label class="text-xs text-muted-foreground">
        Row Gap: {textBuffPanelStyle.gap}px
        <input
          class="w-full mt-1"
          type="range"
          min="0"
      ...
      {#if textBuffPanelStyle.displayMode === "modern"}
        <label class="text-xs text-muted-foreground">
          Name–Value Gap: {textBuffPanelStyle.columnGap}px
          <input
            class="w-full mt-1"
            type="range"
            min="0"
      ...
      <div class="space-y-1">
        {#each buffPriorityIds as buffId, idx (buffId)}
          <div class="flex items-center gap-2 rounded border border-border/60 bg-muted/20 px-2 py-1">
            ...
            <button type="button" class="text-xs px-2 py-0.5 rounded border border-border/60 hover:bg-muted/40 disabled:opacity-50" onclick={() => moveGlobalPriority(buffId, "up")} disabled={idx === 0}>Up</button>
            <button type="button" class="text-xs px-2 py-0.5 rounded border border-border/60 hover:bg-muted/40 disabled:opacity-50" onclick={() => moveGlobalPriority(buffId, "down")} disabled={idx === buffPriorityIds.length - 1}>Down</button>
          </div>
        {/each}
      </div>
    </div>
      ...
      <div class="flex items-center justify-between gap-3">
        <div>
          ...
          <p class="text-xs text-muted-foreground">
            Adds a grid area that shows all Buffs (automatically excludes Buffs already selected in individual mode)
          </p>
        </div>
    ...
    <div class="rounded-lg border border-border/60 bg-card/40 p-4 space-y-4 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.02)]">
      <div class="flex items-center justify-between gap-3">
        <div>
          ...
          <p class="text-xs text-muted-foreground">
            Manage Buff display through groups — icons auto-align in a grid within each group
          </p>
        </div>
        <button
          type="button"
              ...
              <div class="space-y-1">
                ...
                {#each getGroupPriorityIds(group) as buffId, idx (buffId)}
                  <div class="flex items-center gap-2 rounded border border-border/60 bg-muted/20 px-2 py-1">
                    ...
                    <button type="button" class="text-xs px-2 py-0.5 rounded border border-border/60 hover:bg-muted/40 disabled:opacity-50" onclick={() => moveGroupPriority(group.id, buffId, "up")} disabled={idx === 0}>Up</button>
                    <button type="button" class="text-xs px-2 py-0.5 rounded border border-border/60 hover:bg-muted/40 disabled:opacity-50" onclick={() => moveGroupPriority(group.id, buffId, "down")} disabled={idx === getGroupPriorityIds(group).length - 1}>Down</button>
                  </div>
                {/each}
              </div>
              <div class="space-y-2">
